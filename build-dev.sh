# create a tinycorelinux fs with custom .tcz packages (this one is for a build environment)
...
nugget http://tinycorelinux.net/6.x/x86_64/release/CorePure64-6.3.iso \
	http://tinycorelinux.net/6.x/x86_64/tcz/{pkg-config,make,gcc,gcc_base-dev,gcc_libs-dev,gcc_libs,glibc_base-dev,linux-3.16.2_api_headers,fuse,fuse-dev}.tcz \
	-c

# build chain dependencies (remove if you dont need to compile stuff)
...
( cd core ; find | sudo cpio -o -H newc ) | gzip -c > hypercore-dev.gz
